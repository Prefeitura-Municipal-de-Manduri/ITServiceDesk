...
const List = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://${IP.ip}:3001/chamados`)
      .then((response) => {
        setData(response.data.map((item, index) => ({
          ...item,
          position: index === 0 ? "Em atendimento" : `Este é o ${index + 1}º chamado, na lista de atendimento. Há outros ${index} chamados a serem atendidos, Aguarde.`, // 
    ...
    axios
      .get(`http://${IP.ip}:3001/finalizados`)
      .then((response) => {
