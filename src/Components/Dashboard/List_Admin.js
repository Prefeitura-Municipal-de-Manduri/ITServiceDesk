    ...
    axios.get(`http://${IP.ip}:3001/chamados`)
      .then(response => {
        ...
        setData(response.data.map((item, index) => ({
          ...item,
          position: index === 0 ? "Em atendimento" : `${index + 1}º`, 
        })));
...
<Flex mt="-10" flexDirection={{ base: "column", md: "row" }} justifyContent="flex-start" alignItems="flex-start">
  ...
  <Text as="span"fontWeight="bold">Marcelo! 👨🏻‍💻<br/><br/> </Text>
  <Text as="span">Daily tasks:<br/> </Text>
    ...
    <Flex align="center">
      <BsTools />
      <Text ml={1} mr={1}>Att Codes Git</Text>
    </Flex>
